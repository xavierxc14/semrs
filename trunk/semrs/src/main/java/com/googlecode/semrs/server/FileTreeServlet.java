...
public class FileTreeServlet extends HttpServlet {
	...
	File patientFileFolder = null;
	
	public void init(ServletConfig config) throws ServletException{
		//initTree();
	}
	
	public void initTree(String encounterId, String patientId){
		URL resource = this.getClass().getResource("/patient/");
		patientFileFolder = new File(resource.getFile() + patientId + "/" + encounterId + "/");
	

		for(File file : patientFileFolder.listFiles()){
		  if(!file.isDirectory()){
			 files.put("root", file.getAbsolutePath());
			}
		}
	...
	throws ServletException, IOException {
		...
		initTree(encounterId, patientId);
	...
	private String doFileUpload(HttpServletRequest req){
		...
		try {
			...
			while (iter.hasNext()) {
			    ...
			    if (item.isFormField()) {
					...
					FileOutputStream fos = new FileOutputStream(patientFileFolder);
		...
		String retval = "{\"success\":false,\"errors\":{\"filename\":\"Error al subir archivo.\"}}";
	...
	private String doDeleteCmd(String file){
		String retval = "{\"success\":false,\"error\":\"El directorio o archivo no puede ser modificado\"}";
	...
	private String doRenameCmd(String newname, String oldname){
		...
		if(lastSlash == -1){
			retval = "{\"success\":false,\"error\":\"El directorio raiz no puede ser modificado\"}";
		}else{
